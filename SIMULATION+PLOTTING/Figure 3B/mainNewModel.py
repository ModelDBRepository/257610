...
for iter in range(iterations):
    ...
    syn_ports = {'AMPA':1,'NMDA':2,'GABA':3} #how the model sees different receptor types
    f_desired=1.
    f_max=20.
...
#    #multimeter_I_GABA
#    x=0
#    multimeter_I_AMPA_NEG_PYR0 = nest.Create("multimeter")
#    nest.SetStatus(multimeter_I_AMPA_NEG_PYR0,{"withtime":True,"record_from":["V_m","I_GABA","I_AMPA_NEG"]})
#    for x in range(len(PYR_MC0)):
#        nest.Connect(multimeter_I_AMPA_NEG_PYR0,[PYR_MC0[x]])
    ...
    for x in range(len(PYR_MC0)):
        ...
        I_GABA_PYR0_NoDBC_list.append(I_GABA_PYR0)
        ts_GABA_PYR0_NoDBC_list.append(ts_GABA_PYR0)
...
for x in range(len(PYR_MC0)*iterations):
    np.savetxt("I_GABA_PYR0_NoDBC_list{}.txt".format(x), I_GABA_PYR0_NoDBC_list[x], delimiter=",")
    np.savetxt("ts_GABA_PYR0_NoDBC_list{}.txt".format(x), ts_GABA_PYR0_NoDBC_list[x], delimiter=",")
